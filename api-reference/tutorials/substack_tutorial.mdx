...
This app is built on the code from Step 2 and is converted into a Flask app. 
We already have a template ID and are now dynamically seting the input URLs. 
The Flask server listens for HTTP POST requests on the /create-app endpoint.
When it receives a request, it extracts the input_url from the form data, and sends a POST request to the Berri API endpoint 
with the appropriate template_id, user_email, and data_source parameters - the data source changes based on the substack blog post 
The response JSON from the Berri API, which includes the api_endpoint and website_endpoint, is then returned to the caller of the /create-app endpoint.
example response for this substack https://substack.com/inbox/post/109167394
```
{
...
That's it for this tutorial. Here are some future steps you could take to build upon what we've done:

Make this a full-stack app by adding a frontend to call your Flask server. You can use tools like React or Vue to build the frontend. Here's a good starting point: https://replit.com/join/ceztxyemli-ishaan-jaff
Customize the chat.berri.ai interface for your application by adding your own custom branding and keeping only the features you require.
The chat.berri.ai is open-sourced on GitHub. You can check it out here: https://github.com/ClerkieAI/Berri-chat-fe.