---
title: "How to Build a Custom Slackbot to Store Mappings of Channel IDs/Berri API Endpoints and then Call the Berri API Endpoints"
...
TLDR: Want to jump right into the code, checkout the code here: https://berriSlackDemo.ishaan-jaff.repl.co
...
Create the Slack Bolt app object with your signing secret and OAuth settings object.
```python
...
Create a WebClient object with your bot token. This will be used to send messages later.
```python
client = WebClient(token="YOUR_BOT_TOKEN")
```

Define any event handlers that you want to use with your app. In this case, there are two event handlers: one for message events and one for app_mention events.
...
Create a Flask app and a Slack request handler for your app.
```python
flask_app = Flask(__name__)
handler = SlackRequestHandler(app)
```

Define a route for Slack events and use the request handler to handle requests.
```python
